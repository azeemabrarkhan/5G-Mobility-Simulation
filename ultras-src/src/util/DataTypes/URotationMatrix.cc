...
#include <util/DataTypes/URotationMatrix.h>
#include "inet/common/INETMath.h"

using namespace inet;

URotationMatrix::URotationMatrix()
{
    // identity matrix
    matrix[0][0] = 1;
...
URotationMatrix::URotationMatrix(const double matrix[3][3])
{
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
...
URotationMatrix::URotationMatrix(const UEulerAngles& eulerAngles)
{
    UQuaternion q(eulerAngles);
    computeRotationMatrix(q.s, q.v.x, q.v.y, q.v.z);
}

void URotationMatrix::computeRotationMatrix(const double& q0, const double& q1, const double& q2, const double& q3)
{
...
double URotationMatrix::computeDeterminant() const
{
...
Coord URotationMatrix::rotateVector(const Coord& vector) const
{
...
Coord URotationMatrix::rotateVectorInverse(const Coord& vector) const
{
...
UEulerAngles URotationMatrix::toEulerAngles() const
{
    ...
    return UEulerAngles(tz, ty, tx);
}

UQuaternion URotationMatrix::toQuaternion() const
{
