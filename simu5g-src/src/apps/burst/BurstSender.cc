...
void BurstSender::sendBurst()
{
    ...
    simtime_t d = simTime() + interBurstTime_ + videoLength_ + 2;
    if(d < stopTime_ || stopTime_ <= SIMTIME_ZERO) {
        scheduleAt(d, selfBurst_);
    }
}
...
void BurstSender::finish()  {
        recordScalar("packetSize", size_);
        recordScalar("NumPacketsPerVideo", burstSize_);
        recordScalar("NumVideos", idBurst_);
        recordScalar("numSent", idBurst_ * burstSize_);
        recordScalar("bytesSent", idBurst_ * burstSize_ * size_);
        recordScalar("KbsSent", (idBurst_ * burstSize_ * size_)/1024);
}
