...
void BurstReceiver::handleMessage(cMessage *msg)
{
    ...
    string latencyFileName = par("latencyFileName");
    ofstream myfile;
    
    myfile.open(latencyFileName, ios_base::app);
    myfile << simTime();
    myfile << ",";
    myfile << delay;
    myfile << "\n";
...
void BurstReceiver::finish()  {
        recordScalar("numReceived", numReceived_);
        recordScalar("bytesReceived", numReceived_*51200);
}
